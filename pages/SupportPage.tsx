      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center space-y-6">
          <h1 className="text-5xl sm:text-7xl font-black text-gray-900 tracking-tight leading-none">
            Concierge <br />
            <span className="text-eln-primary">{common.support}.</span>
          </h1>
          ...
          <div className="max-w-xl mx-auto relative mt-10">
            ...
            <input 
              ...
              className="w-full pl-14 pr-6 py-5 bg-white border border-gray-100 rounded-[2rem] shadow-xl shadow-gray-200/40 focus:ring-2 focus:ring-eln-primary/20 focus:border-eln-primary transition-all font-medium"
            />
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { icon: MessageCircle, title: t.liveChat, detail: t.liveChatDesc, action: t.startChat, color: "bg-eln-orange-deep" },
            { icon: Mail, title: t.email, detail: t.emailDesc, action: t.sendEmail, color: "bg-eln-primary" },
            { icon: Phone, title: t.phone, detail: t.phoneDesc, action: t.callNow, color: "bg-eln-primary" }
          ].map((contact, i) => (
            <div key={i} className="bg-white p-10 rounded-[2.5rem] border border-gray-100 text-center space-y-6 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group">
              ...
              <button className="w-full py-4 bg-gray-50 text-gray-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-gradient-eln hover:text-white transition-all">
                {contact.action}
              </button>
            </div>
          ))}
        ...
        <motion.div variants={itemVariants} className="space-y-12">
          <div className="flex items-center justify-between">
            <h2 className="text-3xl font-black text-gray-900 flex items-center">
              <HelpCircle className="h-8 w-8 mr-3 text-eln-primary" /> {landing.faqTitle}
            </h2>
          </div>
          
          <div className="grid grid-cols-1 gap-4">
            {filteredFaqs.map((f: any, i: number) => (
              <div key={i} className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden transition-all hover:border-eln-primary/20">
                <button 
                ...
                >
                  ...
                  {openFaqIndex === i ? (
                    <ChevronUp className="h-5 w-5 text-eln-primary" />
                  ) : (
        ...
        {/* Trust Banner */}
        <motion.div variants={itemVariants} className="bg-gradient-eln-deep p-12 rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-8 text-white shadow-2xl shadow-eln-orange-deep/20">
